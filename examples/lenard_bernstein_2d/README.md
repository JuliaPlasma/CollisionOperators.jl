# 2D conservative Lenard–Bernstein operator

Implicit-midpoint particle discretisation of the conservative Lenard–Bernstein
collision operator in 2D velocity space, following Jeyakumar et al. (2024)
(structure-preserving particle method for the Landau operator), specialised to
the Lenard–Bernstein kernel. The density `f_s` is represented in a tensor-product
B-spline finite-element space (via [`Mantis`](https://github.com/JuliaPlasma/Mantis.jl));
particles carry the measure and are advected by a drift whose Lagrange
multipliers enforce exact discrete conservation of momentum and energy.

This is example / research code: it lives outside the package source so the
`CollisionOperators` package itself stays free of plotting and FEM dependencies.
...
See the [package documentation](https://JuliaPlasma.github.io/CollisionOperators.jl/dev/lenard_bernstein/)
